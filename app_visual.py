...
# --- SISTEMA PRINCIPAL ---
    ...
    # --- PESTAÑA 2: INVENTARIO (AHORA CON EDICIÓN DE PRECIOS) ---
    with tab2:
        ...
        # ACORDEÓN 2: EDITAR PRECIOS (¡NUEVO!)
        with st.expander("✏️ Editar Precios de un Producto"):
            if productos and isinstance(productos, list):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    nuevo_precio_c = st.number_input("Costo Proveedor Actualizado ($):", min_value=0.0, value=float(prod_data['precio_compra']), step=1.0)
                with c2:
                    nuevo_precio_v = st.number_input("Precio al Público Actualizado ($):", min_value=0.0, value=float(prod_data['precio_venta']), step=1.0)
